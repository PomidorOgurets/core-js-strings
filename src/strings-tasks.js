...
function removeFirstOccurrences(str, value) {
  ...
  return idx === -1 ? str : str.slice(0, idx) + str.slice(idx + value.length);
}
...
function isPalindrome(str) {
  const strWithNoSpaces = str.split(' ').join('').toLowerCase();
  const alphabet = 'abcdefghijklmnopqrstuvwxyz';
  let strWithNoSpace = '';
  for (let i = 0; i < strWithNoSpaces.length; i += 1) {
    if (alphabet.includes(strWithNoSpaces[i])) {
      strWithNoSpace += strWithNoSpaces[i];
    }
  }
  return strWithNoSpace === strWithNoSpace.split('').reverse().join('');
}
...
function findLongestWord(sentence) {
  ...
  const sentenceLower = sentence;
  let newSentence = '';
  for (let i = 0; i < sentenceLower.length; i += 1) {
    if (alphabet.includes(sentenceLower[i])) {
      newSentence += sentenceLower[i];
    }
  }
  const wordsArray = newSentence.split(' ');
  let result = '';
  for (let j = 0; j < wordsArray.length; j += 1) {
    if (wordsArray[j].length > result.length) {
      result = wordsArray[j];
    }
  }
  return result;
